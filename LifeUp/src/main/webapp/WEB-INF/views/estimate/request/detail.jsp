		...
		<c:forEach items="${contract}" var="salesDoc" varStatus="loop">	
			<c:choose>
				<c:when test="${salesDoc.SALESMAN_ID == sessionScope._USER_INFO_.userId || request.CUSTOMER_ID == sessionScope._USER_INFO_.userId}">				
			    	<li>
			    		<a href="#" data-icon="" onclick="fnContractDetail('${salesDoc.SALESMAN_ID}');" rel="external">
		...
		<c:when test="${sessionScope._USER_INFO_.userType == '2'}">
			<div style="margin-bottom: -15px; margin-right: -10px; text-align: right;">
				<c:if test="${request.STATUS == '0001'}">
					<a href="#" data-role="button" data-icon="edit" data-inline="true" onclick="fnContractWrite(${sessionScope._USER_INFO_.userType});" rel="external">견적남기기</a>
				</c:if>
				<a href="#" data-role="button" data-icon="grid" data-inline="true" onclick="fnList();" rel="external">목록</a>
			</div>
		</c:when>	
		<c:otherwise>
			<div style="margin-bottom: -15px; margin-right: -10px; text-align: right;">
